...
def build_lstm_model():
    model = Sequential()
    model.add(LSTM(128, return_sequences=True, input_shape=(1, 300)))  # Increase the number of LSTM units
...
def preprocess_test_data(df_test):
    ...
    padded_word_vectors_test, _ = convert_to_word_embeddings(df_test, word2Vec_model)

    X_test = np.expand_dims(padded_word_vectors_test, axis=1)
...
# Assuming you have a test directory with the test data
...
model = load_model('lstm_model.h5')  # Uncomment this line if you want to load the trained model

# Predict on the test data
y_pred_test = predict_test_data(model, X_test)

# Display the predictions
pd.set_option('display.max_colwidth', 50)
df_test['Predicted_Label'] = ['positive' if pred == 1 else 'negative' for pred in y_pred_test]
print(df_test[['Text']])
